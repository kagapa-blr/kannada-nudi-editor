...
class TextEditorBackup(QMainWindow):
    def __init__(self):
        super().__init__()
    ...
    def saveFileAsDocx(self):
        ...
        if file_name:
            doc = docx.Document()
            self.addTextToDocx(doc, self.text_edit.document())
            try:
                doc.save(file_name)
    ...
    def addTextToDocx(self, doc, text_document):
        ...
        while not cursor.atEnd():
            ...
            char_text = cursor.block().text()[cursor.positionInBlock():cursor.positionInBlock() + len(cursor.selectedText())]
            ...
            if color.isValid():
                ...
                run.font.color.rgb = docx.shared.RGBColor(*rgb_color)

            run.add_text(char_text)
            cursor.movePosition(QTextCursor.NextCharacter)

            # If the next character has different format, start a new run
            if cursor.atBlockEnd() or cursor.charFormat() != char_format:
                paragraph = doc.add_paragraph()
                run = paragraph.add_run()

    def convertAlignment(self, alignment):
        if alignment == Qt.AlignLeft:
            return docx.enum.text.WD_PARAGRAPH_ALIGNMENT.LEFT
        elif alignment == Qt.AlignRight:
            return docx.enum.text.WD_PARAGRAPH_ALIGNMENT.RIGHT
        elif alignment == Qt.AlignCenter:
            return docx.enum.text.WD_PARAGRAPH_ALIGNMENT.CENTER
        elif alignment == Qt.AlignJustify:
            return docx.enum.text.WD_PARAGRAPH_ALIGNMENT.JUSTIFY
        else:
            return docx.enum.text.WD_PARAGRAPH_ALIGNMENT.LEFT  # Default to left alignment

    # Ensure to import and set up necessary PyQt5 modules (e.g., Qt) for alignment conversion
